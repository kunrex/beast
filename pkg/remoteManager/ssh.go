...
const (
	CMD_OUTPUT_VARIABLE string = "output"
	CMD_ECHO_EXIT_CODE  string = "echo $?"
)
...
func RunCommandsInContainerOnServer(server config.AvailableServer, containerId string, cmds []string) ([]cr.ExecResult, error) {
	results := make([]cr.ExecResult, len(cmds))
	for i, cmd := range cmds {
		result, err := RunCommandInContainerOnServer(server, containerId, cmd)
		if err != nil {
			return results, err
		}
